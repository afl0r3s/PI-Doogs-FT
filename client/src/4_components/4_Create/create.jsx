...
import { getTemperaments, postBreed } from '../../1_actions';
import './create.css';
...
export default function Create() {
    ...
    const handleSubmit = (e) => {
        ...
        if(!input.name || !input.height || !input.weight || !input.life_span || !input.image) alert('One o more fileds are empty !!')
        const dataPostBreed = {
            name: input.name,
            height: input.height,
            weight: input.weight,
    ...
    return (
        ...
        <nav className="navSearch">
            ...
            <div> <span className="appTitle"> &#187; Create a new Breed &#171; </span> </div>
            <div> <Link to={'/home'}> <button className="navSearchHome">Home</button> </Link> </div>
        </nav>
        <form onSubmit={handleSubmit}>
        <fieldset>
            <legend> Create you new Breed </legend>
                <label>Name of breed</label><br/>
                <input type="text" className={errors.name ? 'danger' : 'imputCreateTxt' }  name="name"
                placeholder="Name for new breed.." value={input.name} onChange={handleInputChange}></input>
                {errors.name && ( <p className="danger">{errors.name}</p> )}
                <p></p>
                <label>Height</label><br/>
                <input type="text" className={errors.height ? 'danger' : 'imputCreateTxt' } name="height"
                placeholder="Min - Max" value={input.height} onChange={handleInputChange}></input>
                {errors.height && ( <p className="danger">{errors.height}</p> )}
                <p></p>
                <label>Weight</label><br/>
                <input type="text" className={errors.weight ? 'danger' : 'imputCreateTxt' } name="weight"
                placeholder="Min - Max" value={input.weight} onChange={handleInputChange}></input>
                {errors.weight && ( <p className="danger">{errors.weight}</p> )}
                <p></p>
                <label>Life Span</label><br/>
                <input type="text" className={errors.life_span ? 'danger' : 'imputCreateTxt' } name="life_span"
                ...
                <input type="text"className={errors.image ? 'danger' : 'imputCreateTxt' } name="image"
                ...
                <div>
                    { tempName.map(d => ( <div key={d.id} >{d.name} <span onClick={() => handleRemoveTemperament(d.id)} className="close">[X]</span> </div> )) }
                </div>
        </fieldset>
            <button type="submit" className="buttonSearch"> <i className="gg-search"></i> &nbsp; Save </button>
        </form>
        {
            statePost ? <span>Guardadon con exito</span> : <span>Ocurro un problema</span>
        }
        </div>
    )
}
